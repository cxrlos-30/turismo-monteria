...
async function cargarHoteles() {
  const { data } = await window._supabase.from('hoteles').select('*');
  ...
  if (select) select.innerHTML = "<option>Selecciona un hotel</option>";
  ...
  data.forEach(hotel => {
    ...
    if (select) {
      select.innerHTML += `<option value="${hotel.nombre}">${hotel.nombre}</option>`;
    }
    ...
    card.addEventListener("click", () => {
      abrirModalHotel(hotel.nombre, hotel.imagen, hotel.ubicacion, hotel.descripcion);
    });
...
function seleccionarHotelYReservar(nombreHotel) {
  const select = document.getElementById("hotelSeleccionado");
  if (select) select.value = nombreHotel;
...
function modalHotelReservar() {
  const nombre = document.getElementById("btnReservarDesdeModal").dataset.hotel;
...
async function cargarLugares() {
  const { data } = await window._supabase.from('lugares').select('*');
  ...
  data.forEach(lugar => {
    ...
    `;
    card.addEventListener("click", () => {
      abrirModal(lugar.nombre, lugar.imagen, lugar.ubicacion, lugar.descripcion);
    });
    lista.appendChild(card);
  });
}
...
async function cargarGuias() {
  const { data } = await window._supabase.from('guias').select('*');
...
async function registrarGuia() {
  try {
    ...
    const { error } = await window._supabase.from("guias").insert([{ nombre, telefono, experiencia, imagen: foto }]);

    if (error) { Toast.error("Error al registrar: " + error.message); return; }

    Toast.exito("¡Registro guardado correctamente!");
  } catch (err) {
    Toast.error("Fallo inesperado: " + err.message);
  }
}
...
async function guardarReserva() {
  ...
  if (!user) {
    Toast.aviso("Debes iniciar sesión para reservar");
  ...
  if (hotel === "Selecciona un hotel" || !nombre.trim() || !correo.trim() || !telefono.trim() || !ingreso.trim() || !salida.trim()) {
    Toast.aviso("Debes completar todos los campos antes de reservar");
    return;
  }

  const { error } = await window._supabase.from("reservas_hoteles").insert([{
    user_id: user.id, hotel, nombre, correo, telefono,
    fecha_ingreso: ingreso, fecha_salida: salida
  }]);

  if (error) { Toast.error(error.message); return; }

  Toast.exito("¡Reserva realizada correctamente! 🏨");
...
function mostrarFormularioReserva() {
  document.getElementById("formReserva").classList.toggle("oculto");
...
function cerrarModalGuia() {
  document.getElementById("modalGuia").classList.add("oculto");
}
...
window.addEventListener("scroll", () => {
  ...
  if (!barra) return;
  if (actual <= 20) { barra.classList.remove("ocultar"); return; }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (modal && cerrarModal) {
    window.abrirModal = function(nombre, imagen, ubicacion, descripcion) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (modalHotel && cerrarHotel) {
    window.abrirModalHotel = function(nombre, imagen, ubicacion, descripcion) {
      ...
      document.getElementById("btnReservarDesdeModal").dataset.hotel = nombre;
      modalHotel.style.display = "flex";
    };

    cerrarHotel.addEventListener("click", () => { modalHotel.style.display = "none"; });
    modalHotel.addEventListener("click", (e) => { if (e.target === modalHotel) modalHotel.style.display = "none"; });
  }
});
...
async function cargarMisReservas() {
  ...
  if (!user) { mensaje.style.display = "block"; return; }

  const { data, error } = await window._supabase
    .from("reservas_hoteles").select("*").eq("user_id", user.id);

  if (error) { console.log(error.message); return; }

  if (!data || data.length === 0) { mensaje.style.display = "block"; return; }

  mensaje.style.display = "none";

  data.forEach(reserva => {
    contenedor.innerHTML += `
      <div class="reserva-card" onclick="abrirModalReserva('${reserva.hotel}','${reserva.fecha_ingreso}','${reserva.fecha_salida}','${reserva.nombre}','${reserva.correo}','${reserva.telefono}')">
...
async function eliminarReserva(id) {
  const confirmar = await Toast.confirmar("¿Deseas eliminar esta reserva?");
  if (!confirmar) return;

  const { error } = await window._supabase.from("reservas_hoteles").delete().eq("id", id);

  if (error) { Toast.error(error.message); return; }

  Toast.exito("Reserva eliminada correctamente");
  cargarMisReservas();
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (cerrarModalReserva && modalReserva) {
    cerrarModalReserva.addEventListener("click", () => { modalReserva.style.display = "none"; });
    modalReserva.addEventListener("click", (e) => { if (e.target.id === "modalReserva") modalReserva.style.display = "none"; });
  }
});
